...
public static class PGEditorUtils
{
    ...
    public static T GetDataAsset<T> (string basePath) where T : ScriptableObject
    {
        ...
        string filePath = StringUtils.Format ("{0}/{1}.asset", basePath, fileName);
        T asset;
    ...
    where T : new ()
    {
        ...
        for (int i = 0; i < list.Count; i++)
        {
            ...
            if (itemLabel == "")
                itemLabel = StringUtils.Format ("Element {0}", i);
    ...
    static bool ShowTList<T> (string label, List<T> list, bool expanded, ref Dictionary<object, bool> foldOutStates) where T : new ()
    {
        int indent = EditorGUI.indentLevel;
        int buttonSpacer = 10;
        #region Header Foldout
        ...
        if (list.Count > 0)
        {
            GUIContent content;
            var collapseIcon = '\u2261'.ToString ();
            content = new GUIContent (collapseIcon, "折叠列表");
            masterCollapse = GUILayout.Button (content, EditorStyles.toolbarButton);

            var expandIcon = '\u25A1'.ToString ();
            content = new GUIContent (expandIcon, "展开列表");
            masterExpand = GUILayout.Button (content, EditorStyles.toolbarButton);
        }
        else
        {
        ...
        // Main Add button
        if (GUILayout.Button (new GUIContent ("+", "点击添加"), EditorStyles.toolbarButton))
            list.Add (new T ());
        EditorGUILayout.EndHorizontal ();
        ...
        for (int i = 0; i < list.Count; i++)
        {
            ...
            string itemLabel = PGEditorUtils.GetSerializedObjFieldName<T> (item);
            if (itemLabel == "")
                itemLabel = StringUtils.Format ("第{0}个", i + 1);

            // Get the foldout state. 
            //   If this item is new, add it too (singleton)
            //   Singleton works better than multiple Add() calls because we can do 
            //   it all at once, and in one place.
            bool foldOutState, collapseBools = false;
            ...
            EditorGUILayout.BeginHorizontal ();
            EditorGUI.indentLevel = 0; // Space will handle this for the header
            GUILayout.Space ((indent + 6) * 6); // Matches the content indent
    ...
    public static string GetSerializedObjFieldName<T> (T instance)
    {
        ...
        foreach (System.Reflection.FieldInfo fieldInfo in fields)
            if (fieldInfo.Name.ToLower ().Contains ("name"))
                return ((string) fieldInfo.GetValue (instance)).DeCamel ();
            else if (fieldInfo.Name.ToLower () == "paneltype")
    ...
    static Dictionary<object, bool> listStates = new Dictionary<object, bool> ();
    ...
    public static void FieldInfoField<T> (T instance, System.Reflection.FieldInfo fieldInfo)
    {
        ...
        {
            //获取header的属性
            object[] os = fieldInfo.GetCustomAttributes (true);
            for (int i = 0; i < os.Length; i++)
            {
                if (os[i].GetType () == typeof (UnityEngine.HeaderAttribute))
                {
                    UnityEngine.HeaderAttribute h = (UnityEngine.HeaderAttribute) os[i];
                    label = h.header;
        ...
        #region 自定义类型
        // else if (fieldInfo.FieldType == typeof (PanelType))
        // {
        //     PanelType val = (PanelType) Enum.Parse (typeof (PanelType), fieldInfo.GetValue (instance).ToString ());
        //     val = (PanelType) EditorGUILayout.EnumPopup (label, val);
        //     fieldInfo.SetValue (instance, val);
        //     return;
        // }
        // else if (fieldInfo.FieldType == typeof (PanelLayerType))
        // {
        //     PanelLayerType val = (PanelLayerType) Enum.Parse (typeof (PanelLayerType), fieldInfo.GetValue (instance).ToString ());
        //     val = (PanelLayerType) EditorGUILayout.EnumPopup (label, val);
        //     fieldInfo.SetValue (instance, val);
        //     return;
        // }
        #endregion
    ...
    private static LIST_BUTTONS AddFoldOutListItemButtons ()
    {
        ...
        if (upPressed == true)
            return LIST_BUTTONS.Up;
        if (downPressed == true)
            return LIST_BUTTONS.Down;
        if (removePressed == true)
            return LIST_BUTTONS.Remove;
        if (addPressed == true)
            return LIST_BUTTONS.Add;

	...
	public class PathConstraint : IUpdatable
	{
		...
		public void Update()
		{
			...
			while (num11 < count)
			{
				...
				if (flag5)
				{
					float num17 = exposedList2.Items[num11];
					if (num17 != 0f)
					{
						float num18 = ((float)Math.Sqrt(num15 * num15 + num16 * num16) / num17 - 1f) * num + 1f;
						bone2.a *= num18;
						bone2.c *= num18;
					}
				}
				...
				if (flag2)
				{
					...
					float c = bone2.c;
					float d = bone2.d;
					float num19 = flag4 ? array[num12 - 1] : ((exposedList.Items[num11 + 1] != 0f) ? MathUtils.Atan2(num16, num15) : array[num12 + 2]);
					num19 -= MathUtils.Atan2(c, a) - offsetRotation * ((float)Math.PI / 180f);
					float num20;
					float num21;
					if (flag6)
					{
					...
					if (num19 > (float)Math.PI)
					{
						num19 -= (float)Math.PI * 2f;
					}
					else if (num19 < -(float)Math.PI)
					{
						num19 += (float)Math.PI * 2f;
					}
					num19 *= num;
					num20 = MathUtils.Cos(num19);
					num21 = MathUtils.Sin(num19);
					bone2.a = num20 * a - num21 * c;
					bone2.b = num20 * b - num21 * d;
					bone2.c = num21 * a + num20 * c;
					bone2.d = num21 * b + num20 * d;
				}
				num11++;
				num12 += 3;
			}

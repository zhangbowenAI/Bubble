...
public class IJsonData<T>: SingletonBase<T> where T : new()
{
    ...
    public string InitData(string fileName, bool isCreateAnotherData = false, bool isEncryptData = false)
	{
        ...
        //检查是否有另外的数据文件
        if (IsCreateAnotherData && NewFileTool.IsFileExists(DataFileName))
        {
            jsonStr = NewFileTool.ReadAllFile(DataFileName);
        ...
        else
        {
            ...
            {
                NewFileTool.createORwriteFile(DataFileName, IsEncryptData ? DesCode.EncryptDES(jsonStr, DesCode.PassWord) : jsonStr);
            }
        }
